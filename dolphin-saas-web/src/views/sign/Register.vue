    ...
    <a-form id="formRegister" ref="formRegister" :form="form">
      ...
      <a-form-item>
        <a-input
          ...
          v-decorator="['mobile', {rules: [{required: true, message: $t('user.phone-number.required')},{ pattern: /^1[23456789]\d{9}$/ ,message: $t('user.login.mobile.placeholder')} ], validateTrigger: ['change', 'blur']}]"
        >
          <a-select slot="addonBefore" size="large" defaultValue="+86" disabled>
    ...
    handleSubmit() {
      ...
      validateFields({ force: true }, (err, values) => {
        if (!err) {
          ...
          const hide = $message.loading('账户注册中..', 0)
    ...
    getCaptcha(e) {
      ...
      validateFields(['mobile'], { force: true }, (err, values) => {
        if (!err) {
          ...
          getCode({ phoneNumber: values.mobile })
            .then(({ status, msg }) => {
              if (status == 1) {
    ...
    loginSuccess(res) {
      ...
      setTimeout(() => {
        this.$notification.success({
          message: '提示',
          description: `注册成功，请完善下面信息`,
        })
      }, 1000)
    },
    requestFailed(err) {
      console.log('err', err)
      this.$message.error(err || '请求出现错误，请稍后再试')
      this.registerBtn = false
    },
  },
}
